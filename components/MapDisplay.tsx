...
declare global {
  namespace google.maps {
      ...
      class Marker { constructor(opts?: MarkerOptions); setMap(map: Map | null): void; setPosition(latLng: LatLng | { lat: number; lng: number }): void; setIcon(icon: any): void; setZIndex(zIndex: number): void; setOpacity(opacity: number | null): void; addListener(eventName: string, handler: Function): google.maps.MapsEventListener; }
      interface MarkerOptions { position: { lat: number; lng: number; } | LatLng; map?: Map; title?: string; icon?: any; anchor?: Point; zIndex?: number; }
  ...
  useEffect(() => {
    ...
    results.forEach((point, i) => {
        ...
        if (marker) {
            ...
            marker = new window.google.maps.Marker({ position: { lat: point.lat, lng: point.lng }, map });
            markersRef.current[i] = marker;
        }
        marker.setIcon(createMarkerIcon(point.rank, isSelected, isDimmed));
        marker.setZIndex(isSelected ? 100 : point.rank);
  ...
  if (!mapsApiKey || apiKeyError) {
      return (
          <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 p-8 text-center">
              ...
              {apiKeyError && <p className="text-red-500 mb-4 font-medium">{apiKeyError}</p>}
